        ...
        (quote! {
                ...
                #wasm_bindgen::JsValue
            {
                fn from(value: #name) -> Self {
                    ...
                    #[cfg(all(target_family = "wasm", not(any(target_os = "emscripten", target_os = "wasi"))))]
                    extern "C" {
                        fn #new_fn(ptr: u32) -> u32;
                    }

                    #[cfg(not(all(target_family = "wasm", not(any(target_os = "emscripten", target_os = "wasi")))))]
...
impl ToTokens for ast::StructField {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        (quote! {
            #[automatically_derived]
            const _: () = {
                #[cfg_attr(all(target_family = "wasm", not(any(target_os = "emscripten", target_os = "wasi"))), no_mangle)]
                #[doc(hidden)]
